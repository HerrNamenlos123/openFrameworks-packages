...
def get_install_dir(package_name):
    return os.sep.join([get_working_dir(package_name), 'install'])
...
def build_generic_cmake_project(package_name, cmake_args = [], cmake_module_paths = [], cmake_module_path_root = '', cmake_args_debug = [], cmake_args_release = []):
    ...
    args_debug.append(f'-DCMAKE_INSTALL_PREFIX={get_install_dir(package_name)}')
    ...
    args_release.append(f'-DCMAKE_INSTALL_PREFIX={get_install_dir(package_name)}')
